...
object RMFUtil {
  ...
  /** get all sub-types of '''aType''' which are '''allowed''' by the given
   * functor.  note that it is up to the '''allow''' functor to decide whether
   * or not '''aType''' is included in the `TreeSet`.
   */
