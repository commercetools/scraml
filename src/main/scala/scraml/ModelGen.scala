...
final case class ModelGenParams(raml: File,
  ...
  def allLibraries: List[LibrarySupport] = jsonSupport.map {
...
final case class ModelGenContext(packageName: String,
  ...
  def getSubTypes: Iterator[AnyType] = {
    objectType.getSubTypes.asScala.filter(_.getName != objectType.getName).iterator ++
      api.scalaExtends.find {
        case (_, typeValue) => typeValue.getName == objectType.getName
...
object ModelGen {
  ...
  def scalaTypeRef(apiType: AnyType,
                   optional: Boolean,
                   typeName: Option[String] = None,
                   defaultAnyTypeName: String): Option[TypeRef] = {
    lazy val mappedType = apiType match {
      ...
      case array: ArrayType =>
        ...
        TypeRefDetails(Type.Apply(typeFromName(arrayType), List(scalaTypeRef(array.getItems, false, itemTypeOverride, defaultAnyTypeName).map(_.scalaType)).flatten), None, Some(Term.Select(packageTerm(arrayType), Term.Name("empty"))))
      ...
      case union: UnionType => TypeRefDetails(Type.Apply(Type.Name("Either"), union.getOneOf.asScala.flatMap(scalaTypeRef(_, optional, None, defaultAnyTypeName)).map(_.scalaType).toList))
